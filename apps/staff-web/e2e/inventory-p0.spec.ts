...
test('WS-34-B9 deterministic Staff Web P0 journey reaches real inventory authorities', async ({ page }) => {
  ...
  await page.getByLabel('Artículo / presentación').selectOption({ index: 1 }); await page.getByLabel('Recibido', { exact: true }).fill('2'); await page.getByLabel('Aceptado', { exact: true }).fill('2'); await page.getByLabel('Costo unitario', { exact: true }).fill('20'); await page.getByLabel('Código de lote').fill('LOT-RECEIVED-E2E'); await page.getByLabel('Fabricación').fill('2026-08-01'); await page.getByLabel('Caducidad').fill('2027-08-01'); await page.getByLabel('Consumo preferente').fill('2027-07-01'); await page.getByRole('button', { name: 'Agregar línea' }).click();
  ...
  await page.getByLabel('Código del lote preparado').fill('LOT-PREPARED-E2E'); await page.getByLabel('Fabricación del lote').fill('2026-09-12'); await page.getByLabel('Caducidad del lote').fill('2027-09-12'); await page.getByLabel('Consumo preferente del lote').fill('2027-08-12');
