...
export function StaffShell({ children }: PropsWithChildren) {
  ...
  const {
    tenant, organization, location, locations, roles, permissions, selectHome, selectLocation,
  } = useStaffContext();
    ...
    <div className="staff-app">
      <header className="shell-header">
        ...
        <div className="context-strip" aria-label="Contexto operativo actual">
          <div><small>Tenant</small><strong>{tenant?.name ?? 'No disponible'}</strong></div>
          <span className="context-divider" aria-hidden="true"></span>
          <div><small>Organización</small><strong>{organization?.name ?? 'No disponible'}</strong></div>
          <span className="context-divider" aria-hidden="true"></span>
          <label>
            <span>Sucursal</span>
            {locations.length > 1 ? (
              <select aria-label="Sucursal operativa" value={location?.id ?? ''} onChange={(event) => selectLocation(Number(event.target.value))}>
        ...
        <div className="shell-actions">
          ...
          <div className="user-menu">
            ...
            <div><strong>{identity?.display_name}</strong><small>{roles.join(' · ') || 'Personal'}</small></div>
          </div>
      ...
      <div className="shell-body">
        <aside className="shell-sidebar">
          <nav aria-label="Espacios de trabajo">
            <NavLink to="/" end onClick={selectHome}><span aria-hidden="true">⌂</span>Inicio</NavLink>
