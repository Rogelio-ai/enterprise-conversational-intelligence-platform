...
export interface InventoryValuationLine { id:number; inventory_item_id:number; quantity_as_of:string; evidence_status:'RESOLVED'|'COST_NON_DERIVABLE'|'CURRENCY_MISMATCH'; cost_revision_id:number|null; unit_cost_evidence:string|null; cost_currency_evidence:string|null; line_value:string|null }
export interface InventoryValuationSnapshot { id:number; location_id:number; warehouse_id:number; status:'FINALIZED'; valuation_method:'STANDARD_COST'; as_of:string; movement_cursor:number; currency:string|null; derivable_total_value:string|null; non_derivable_line_count:number; cost_visible:boolean; created_at:string; lines:InventoryValuationLine[] }
