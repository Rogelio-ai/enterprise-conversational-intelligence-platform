...
export interface InventoryItemOption { id: number; code: string; name: string; base_uom: string; status: string; lot_tracking_policy?: 'OPTIONAL'|'REQUIRED'; date_tracking_policy?: 'NONE'|'EXPIRY'|'BEST_BEFORE'|'BOTH' }
export interface ReplenishmentPolicy { id:number; location_id:number; warehouse_id:number; inventory_item_id:number; status:'ACTIVE'|'INACTIVE'; minimum_quantity:string|null; target_quantity:string|null; source_uom:string; on_hand:string; on_order:string; suggested_quantity:string; version:number; updated_at:string }
export interface InventoryLot { id:number; location_id:number; warehouse_id:number; inventory_item_id:number; origin_type:'GOODS_RECEIPT'|'PREPARATION_BATCH'; goods_receipt_line_id:number|null; preparation_batch_output_id:number|null; lot_code:string; origin_at:string; manufacture_date:string|null; expiry_date:string|null; best_before_date:string|null; date_state:'VALID'|'APPROACHING'|'EXPIRED'|null; original_quantity:string; balance:string; source_quantity:string; source_uom:string; base_uom_evidence:string; cost_evidence_status:string; unit_cost_evidence:string|null; cost_currency_evidence:string|null; cost_visible:boolean; status:string }
export interface InventoryCostLayer { id:number;warehouse_id:number;inventory_item_id:number;inventory_lot_id:number|null;source_inventory_lot_id:number|null;origin_type:'GOODS_RECEIPT'|'PREPARATION_OUTPUT'|'TRANSFER';origin_id:number;origin_layer_id:number|null;origin_at:string;original_quantity:string;remaining_quantity:string;unit_cost:string|null;currency:string|null;status:'ACTIVE'|'DEPLETED';cost_visible:boolean }
export interface InventoryValuationLine { id:number; inventory_item_id:number; cost_layer_id?:number; quantity_as_of:string; evidence_status:'RESOLVED'|'COST_NON_DERIVABLE'|'FIFO_NON_DERIVABLE'|'CURRENCY_MISMATCH'; cost_revision_id:number|null; unit_cost_evidence:string|null; cost_currency_evidence:string|null; line_value:string|null }
export interface InventoryValuationSnapshot { id:number; location_id:number; warehouse_id:number; status:'FINALIZED'; valuation_method:'STANDARD_COST'|'FIFO'; as_of:string; movement_cursor:number; layer_cursor:number; currency:string|null; derivable_total_value:string|null; non_derivable_line_count:number; cost_visible:boolean; created_at:string; lines:InventoryValuationLine[] }
export interface InventoryTransferAllocation {cost_layer_id:number;quantity:string;unit_cost:string|null;extended_cost:string|null;currency:string|null;allocation_order:number}
export interface InventoryTransferLine {id:number;line_number:number;inventory_item_id:number;sent_quantity:string;received_quantity:string;remaining_in_transit:string;source_movement_id:number|null;source_allocations:InventoryTransferAllocation[];receipts:Array<{sequence:number;source_layer_id:number;destination_layer_id:number;destination_movement_id:number;quantity:string;unit_cost:string|null;extended_cost:string|null;currency:string|null}>}
export interface InventoryTransfer {id:number;location_id:number;source_warehouse_id:number;destination_warehouse_id:number;status:'DRAFT'|'SUBMITTED'|'IN_TRANSIT'|'RECEIVED'|'CANCELLED';reference:string|null;version:number;submitted_at:string|null;shipped_at:string|null;received_at:string|null;cancelled_at:string|null;cost_visible:boolean;lines:InventoryTransferLine[]}
export interface PreparationRecipeComponent { id: number; inventory_item_id: number; line_number: number; expected_quantity: string; source_uom: string; normalized_expected_quantity: string; base_uom_evidence: string; yield_basis: boolean }
export interface PreparationRecipeVersion { id: number; location_id: number; output_inventory_item_id: number; revision: number; status: 'ACTIVE'|'INACTIVE'; publication_status: 'PUBLISHED'; expected_output_quantity: string; output_source_uom: string; normalized_expected_output_quantity: string; output_base_uom_evidence: string; expected_yield: string; effective_from: string; effective_to: string|null; published_at: string; components: PreparationRecipeComponent[] }
export interface PreparationBatchInput { id: number; recipe_component_id: number; inventory_item_id: number; source_quantity: string; source_uom: string; normalized_quantity: string; base_uom_evidence: string; evidence_status: string; standard_unit_cost_evidence?: string|null; extended_material_cost?: string|null; cost_currency_evidence?: string|null }
export interface PreparationBatch { id: number; location_id: number; warehouse_id: number; recipe_version_id: number; status: 'DRAFT'|'IN_PROGRESS'|'COMPLETED'|'CANCELLED'; version: number; reference: string|null; expected_yield: string; actual_yield: string; yield_variance: string; cost_evidence_status: 'RESOLVED'|'COST_NON_DERIVABLE'; material_cost: string|null; prepared_unit_material_cost: string|null; cost_currency_evidence: string|null; cost_visible: boolean; inputs: PreparationBatchInput[]; output: { id:number; inventory_item_id:number; source_quantity:string; source_uom:string; normalized_quantity:string; base_uom_evidence:string; evidence_status:string; allocated_material_cost?:string|null; unit_material_cost?:string|null; cost_currency_evidence?:string|null }; movements: Array<{id:number;movement_type:string;quantity:string}>; created_at:string; started_at:string|null; completed_at:string|null; cancelled_at:string|null }
export interface GoodsReceiptLine { id: number; inventory_item_id: number; purchase_order_line_id: number | null; accepted_quantity: string; rejected_quantity: string; received_quantity: string; source_uom: string; unit_cost: string; currency: string; lot_code?:string|null; manufacture_date?:string|null; expiry_date?:string|null; best_before_date?:string|null; evidence_status: string; stock_movement_id: number | null }
