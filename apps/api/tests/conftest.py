...
def sql_connection(integration_settings: Settings):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                'DELETE FROM onboarding_staff_continuations WHERE tenant_id IN '
                '(SELECT id FROM tenants WHERE slug LIKE %s)',
                (f'{prefix}%',),
            )
            cursor.execute(
                'DELETE FROM onboarding_imports WHERE tenant_id IN '
                '(SELECT id FROM tenants WHERE slug LIKE %s)',
                (f'{prefix}%',),
            )
            cursor.execute(
            ...
            try:
                for table in ('stock_movements', 'restaurant_order_consumptions'):
                    cursor.execute(
                        f'DELETE FROM {table} WHERE tenant_id IN '
                        '(SELECT id FROM tenants WHERE slug LIKE %s)',
                        (f'{prefix}%',),
            ...
            cursor.execute(
                'DELETE FROM menu_section_external_mappings WHERE tenant_id IN '
                '(SELECT id FROM tenants WHERE slug LIKE %s)',
                (f'{prefix}%',),
            )
            cursor.execute(
            ...
            cursor.execute(
                'DELETE FROM menu_external_mappings WHERE tenant_id IN '
                '(SELECT id FROM tenants WHERE slug LIKE %s)',
                (f'{prefix}%',),
            )
            cursor.execute(
            ...
            cursor.execute(
                'DELETE FROM product_category_external_mappings WHERE tenant_id IN '
                '(SELECT id FROM tenants WHERE slug LIKE %s)',
                (f'{prefix}%',),
            )
            cursor.execute(
