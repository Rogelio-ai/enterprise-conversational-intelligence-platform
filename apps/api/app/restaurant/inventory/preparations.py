...
async def project_batch(db: AsyncSession, value: PreparationBatch, cost_visible: bool) -> dict:
    ...
    projected_output = {'id': output.id, 'inventory_item_id': output.inventory_item_id, 'source_quantity': _decimal(output.source_quantity), 'source_uom': output.source_uom, 'normalized_quantity': _decimal(output.normalized_quantity), 'base_uom_evidence': output.base_uom_evidence, 'evidence_status': output.evidence_status, 'lot_code': output.lot_code, 'manufacture_date': output.manufacture_date, 'expiry_date': output.expiry_date, 'best_before_date': output.best_before_date}
...
async def create_batch(
    ...
    output_lot_code: str | None = None, manufacture_date=None,
    expiry_date=None, best_before_date=None,
) -> dict:
    ...
    output_item = items[recipe.output_inventory_item_id]
    if not output_lot_code and output_item.lot_tracking_policy == 'REQUIRED':
        raise PreparationError('Lot code is required by prepared Inventory Item policy')
    if not output_lot_code and any((manufacture_date, expiry_date, best_before_date)):
        raise PreparationError('Tracked dates require an output lot code')
    if output_lot_code:
        b10.validate_lot_dates(output_item, manufacture_date, expiry_date, best_before_date)
